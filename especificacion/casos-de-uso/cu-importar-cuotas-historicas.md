...
**Reglas de importación:** cada período histórico se crea con importe `500`, recargo `100` y vencimiento el día 10 del mes. Una celda mensual verdadera genera una cuota pagada y un pago histórico. Una celda falsa genera una cuota impaga, salvo que tenga una forma de pago cargada en la misma columna, en cuyo caso se da por pagada. La forma de pago `efectivo` se mapea a efectivo y `MP` a billetera virtual.
