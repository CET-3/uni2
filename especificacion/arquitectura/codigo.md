...
- En producción sobre Vercel, las conexiones a Supabase no deben quedar persistentes entre requests. El handler serverless se limita a exponer la aplicación HTTP: no consulta el esquema, no ejecuta migraciones y no carga datos. Ver [Despliegue en Vercel](despliegue.md).
