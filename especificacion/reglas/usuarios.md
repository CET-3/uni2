...
| Administrador de permisos | Experiencia administrativa y auditoría | Alta, consulta y edición de usuarios; alta, consulta, edición y borrado de grupos | No edita superusuarios ni puede asignar `Administrador de la app` |
...
Los permisos efectivos `auth.add_group`, `auth.change_group` y
`auth.delete_group` habilitan respectivamente el alta, la edición y el borrado
de grupos. No existen excepciones adicionales basadas en el nombre del grupo.
